...
const actions = {
	async authentication({ commit }, logInfo) {
		await db.auth.signInWithEmailAndPassword(logInfo.email, logInfo.password)
		.then(function(user) {
	...
	async signUp({ commit }, logInfo) {
		await db.auth.createUserWithEmailAndPassword(logInfo.email, logInfo.password)
		.then(function(user) {
	...
	async logOut({ commit }) {
		await db.auth.signOut().then(function() {
	...
	async updateDisplayName(newName) {
		await db.auth.currentUser.updateProfile({
			displayName: newName
		}).then(function() {
	...
	async deleteAccount({ commit }) {
		await db.auth.currentUser.delete().then(function() {
			commit('authenticate', false)
		}).catch(function(error) {
