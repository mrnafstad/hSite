...
def d_i_arr( mass, beta_arr, L_arr, z_ssb_arr ):
	...
	delta_c4 = []
	for L in  L_arr:
		filename = "Numbers\Symmetron\Sym2\M%.15fz_ssb%.3fL%.3fbeta%.3f.txt" % (np.log10(1e14), 0.103, L, 0.5)
		file = open(filename, "r")
		i = 0
		for j in file.readlines():
			try:
			...
			if i == 0:
				...
				rvirrta4.append(value)
				i += 1
			elif i == 5:
				avirata4.append(value)
				i += 1
			elif i == 6:
				avir4.append(value)
				i += 1
				...
				delta_c4.append(value)
				i = 0
					

		file.close()
	return delta_c1, delta_c2, delta_c3, delta_c4
...
L_ind = np.abs(L_list - 5).argmin()
L_close = L_list[L_ind]

L_ind2 = np.abs(L_list - 1).argmin()
L_close2 = L_list[L_ind2]


z_ssb_ind = np.abs((z_ssb_list - 0.103)).argmin()
z_ssb_close = z_ssb_list[z_ssb_ind]

z_ssb_ind2 = np.abs((z_ssb_list - 1)).argmin()
z_ssb_close2 = z_ssb_list[z_ssb_ind2]

mass_ind = np.abs((masses - 1e12)).argmin()
mass_close = masses[mass_ind]

delta_i_L, delta_i_z, delta_i_beta, delta_i_L2 = d_i_arr(mass_close, beta_list, L_list, z_ssb_list)


figs(1e14, z_ssb_close, 0.5, L_close, "A", "red", delta_i_L2[L_ind])
figs(mass_close, z_ssb_close, 0.5, 1, "B", "blue", delta_i_L[L_ind])
figs(1e14, z_ssb_close2, 0.5, L_close2, "C", "green", delta_i_L[L_ind2])

savefigs(r"$\beta = 0.5$", "LargeDens")
